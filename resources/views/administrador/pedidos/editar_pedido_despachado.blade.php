...
@section('js')
    ...
    <script>
        ...
        $('#botonregistrarpedido').on('click', function() {
            ...
            $.ajax({
                url: "{{route('administrador.pedidos.administrador.agregarProducto.despachados', ':id')}}".replace(':id', "{{ $pedido->numero_pedido }}"),
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        function eliminarProductoPedido(button) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('administrador.pedidos.administrador.eliminarProducto.despachados', ':id') }}".replace(':id', pedidoId),
                        type: 'DELETE',
                        dataType: 'json',
                        data: {
                            _token: "{{ csrf_token() }}"
